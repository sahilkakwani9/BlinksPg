...
const useMeteora = (poolAddress: string) => {
  ...
  const getPollInstance = async () => {
    try {
      console.log("got the call here", connection);

      console.log("poolAddress", poolAddress);

      return await AmmImpl.create(connection, new PublicKey(poolAddress));
    } catch (error) {
  ...
  const fetchQoute = async (amount: BN, out?: boolean) => {
    try {
      ...
      const constantProductPool = await getPollInstance();
      console.log("constant product pool", constantProductPool);
